...
CATEGORY_OPTIONS = (
    ("fill_in_gaps", _("Fill in gaps Quiz")),
    ("true_false", _("True or False Quiz")),
    ("MCQ", _("MCQ Quiz")),
)
...
class Progress(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("User score records")
...
"""
    ...
    # Check if the score_to_add is within the valid range
     if abs(score_to_add) not in [0, 100]:
        return _("error"), _("Invalid score update. Must be 0 or 100")

    # Update the score based on the score_to_add
     self.score += score_to_add
     self.save()

    # Check if score_to_add is an integer
     if not isinstance(score_to_add, int):
        return _("error"), _("Invalid score update. Score must be an integer")

    # Update the time spent
     if self.time_spent is None:
       self.time_spent = timezone.timedelta(seconds=0)
     self.time_spent += timezone.timedelta(seconds=question.time_to_answer)
     self.save()

    # Update the streak count
     if score_to_add > 0:
        # If score_to_add is positive, increment the streak count
        self.streak_count += 1
     else:
        # If score_to_add is non-positive, reset the streak count
        self.streak_count = 0
     self.save()

    # Search for existing score entry for the quiz
     to_find = re.escape(str(question.quiz)) + r",(?P<score>\d+),"

     match = re.search(to_find, self.score, re.IGNORECASE)

     if match:
        # If a match is found, update the existing score entry
        updated_score = int(match.group("score")) + abs(score_to_add)
        new_score = ",".join(
            [str(question.quiz), str(updated_score), ""]
        )

        # Replace the old score entry with the updated one
        self.score = self.score.replace(match.group(), new_score)
     else:
        # If no match is found, add a new score entry
        self.score += ",".join(
            [str(question.quiz), str(score_to_add), ""]
        )
    ...
    # Return success message
     return _("success"), _("Score updated successfully")"""
...
class Sitting(models.Model):
    ...
    lesson = models.ForeignKey(
        Lesson, null=True, verbose_name=_("Lesson"), on_delete=models.CASCADE
    )
    ...
    def add_to_score(self, question, answer):
     if question.check_answer(answer):  # Check if answer is correct
      self.current_score += 100  # Award 100 points for a correct answer
      self.save()
    def update_score(self, question, answer):
        ...
        if not self.start:
            ...
            self.time_spent = datetime.now() - self.start
    ...
    def calculate_score(self):
        ...
        answered_questions = json.loads(self.user_answers)
        total_questions = self.get_max_score()

        if len(answered_questions) < total_questions:
            return _("error"), _("All questions must be answered to calculate the score.")

        # Calculate the number of correct answers
        correct_answers = 0
        for question_id, user_answer in answered_questions.items():
            try:
                question = Question.objects.get_subclass(id=question_id)
            except ObjectDoesNotExist:
                return _("error"), _("Question does not exist.")
        ...
        # Update the current score in the Sitting model
        ...
        return _("success"), _("Quiz score calculated successfully.")
